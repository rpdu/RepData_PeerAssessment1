...
Load file into data frame. Aggregate data to report the mean and median total number of steps taken per day.
...
df_daily_summ <- df_activity %>% group_by(date) %>% summarise(sum_steps = sum(steps, na.rm = TRUE) ,mean_steps = mean(steps, na.rm = TRUE), med_steps= median(steps, na.rm = TRUE))
names(df_daily_summ)<-c("Date","Total steps", "Average Steps", "Median Steps")

df_daily <- aggregate(df_activity$steps, by=list(df_activity$date), FUN=sum, na.rm=TRUE)
names(df_daily)<-c("date","steps")
knitr::kable(df_daily_summ)
```
...
Q.1  Make a time series plot of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis)
...
max(df_interval_avg$mean_steps)
```
...
```{r report_wo_na, results='asis', echo=TRUE}
df_daily_summ_wo_na <- df_activity_wo_na %>% group_by(date) %>% summarise(sum_steps = sum(steps, na.rm = TRUE) ,mean_steps = mean(steps, na.rm = TRUE), med_steps= median(steps, na.rm = TRUE))
names(df_daily_summ_wo_na)<-c("Date","Total steps", "Average Steps", "Median Steps")

knitr::kable(df_daily_summ_wo_na)
```
...
main="Histogram of the total number of steps taken each day (without NÁ)",
xlab="number of steps taken each day")
```
